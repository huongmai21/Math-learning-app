...
const ExamList = () => {
  ...
  const [filters, setFilters] = useState({ educationLevel: '', subject: '', status: '', difficulty: '' });
  ...
  const loadExams = async (pageToLoad = 1) => {
    try {
      ...
      toast.error('Lỗi khi tải danh sách đề thi: ' + (err.message || 'Vui lòng thử lại.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFollow = async (examId) => {
    try {
      ...
      toast.error('Quan tâm bài thi thất bại: ' + (err.message || 'Vui lòng thử lại.'));
    }
  };
  ...
  const getSubjectLabel = (subject) => {
    switch (subject) {
      ...
      case 'advanced_math': return 'Toán cao cấp';
      case 'calculus': return 'Giải tích';
      case 'algebra': return 'Đại số';
      case 'probability_statistics': return 'Xác suất thống kê';
      case 'differential_equations': return 'Phương trình vi phân';
  ...
  return (
    <div className="exam-list-container">
      ...
      <div className="filters">
        ...
        <button onClick={() => setFilters({ educationLevel: '', subject: '', status: '', difficulty: '' })}>
          Xóa bộ lọc
        </button>
      </div>

      {(!filters.educationLevel && !filters.subject && !filters.status && !filters.difficulty && !searchQuery) && (
            ...
            <div className="exam-grid">
              {recommendedExams.map((exam) => (
                <div key={exam._id} className="exam-card">
                  ...
                  <Link to={`/exams/${exam._id}`} className="btn">Xem chi tiết</Link>
                </div>
              ))}
            </div>
          ) : (
          ...
          {exams.length > 0 ? (
            exams.map((exam) => (
              <div key={exam._id} className="exam-card">
                ...
                <p>Thời gian bắt đầu: {new Date(exam.startTime).toLocaleString()}</p>
                <p>Thời gian kết thúc: {new Date(exam.endTime).toLocaleString()}</p>
                <p>Trạng thái: {getStatus(exam.startTime, exam.endTime)}</p>
                <p>Lượt tham gia: {exam.attempts}</p>
                {user && (
                  <button onClick={() => handleFollow(exam._id)}>
                    {exam.followers.includes(user._id) ? 'Đã quan tâm' : 'Quan tâm'}
                  </button>
                )}
                {getStatus(exam.startTime, exam.endTime) === 'Đang diễn ra' && (
                  <Link to={`/exams/${exam._id}`} className="btn">Tham gia</Link>
                )}
                {getStatus(exam.startTime, exam.endTime) === 'Kết thúc' && (
                  <Link to={`/exams/${exam._id}/answers`} className="btn">Xem đáp án</Link>
                )}
                <button onClick={() => fetchExamLeaderboard(exam._id)} className="btn">Xem bảng xếp hạng</button>
