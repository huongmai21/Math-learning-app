...
exports.bookmarkPost = asyncHandler(async (req, res, next) => {
  ...
  // Kiểm tra xem người dùng đã đánh dấu bài đăng chưa
  ...
  res.status(200).json({
    success: true,
    data: { bookmarks: post.bookmarks, isBookmarked: !isBookmarked },
  })
})

// Lấy bài đăng đã đánh dấu của người dùng
exports.getBookmarkedPosts = asyncHandler(async (req, res, next) => {
  ...
  const total = await Post.countDocuments({ bookmarks: req.user.id })
